...
def build_model():
    ...
    return compiler, fc2, difficulty, batched_labels, loss_mse, batch_size_op


def train_with_val(difficulty, batched_labels, train_mse):  # difficulty dtype=float32
    ...
    batched_loss_mse = tf.reduce_mean(tf.square(batched_labels - difficulty))
    loss = tf.reduce_mean(batched_loss_mse) + decay_loss

    # Exponential decay learning rate
    decayed_rate = tf.train.exponential_decay(hyper.learning_rate, global_step, 200, 0.65, staircase=True)
...
def do_train():
    ...
    (compiler, unscaled_difficulty, difficulty, batched_labels, loss_mse, batch_size_op) = build_model()
    (loss, global_step, train_step, summary_op) = train_with_val(difficulty, batched_labels, loss_mse)

    # summaries for loss
    val_summary_op = tf.summary.scalar('loss', loss_mse)
    ...
    with tf.Session() as sess:
        ...
        val_step_counter = 0
        shuffled = zip(td.epochs(train_set, hyper.num_epochs), td.epochs(val_set, hyper.num_epochs))
        ...
        for epoch, (train_shuffled, val_shuffled) in enumerate(shuffled, 1):
            for step, batch in enumerate(td.group_by_batches(train_shuffled, hyper.batch_size), 1):
                ...
                (_, loss_value, summary, gstep, actual_bsize) = sess.run([train_step, loss, summary_op, global_step,
                                                  batch_size_op], train_feed_dict)
            ...
            for batch in td.group_by_batches(val_shuffled, hyper.batch_size):
                ...
                mse_value, actual_bsize, val_summary = sess.run([loss_mse, batch_size_op, val_summary_op], feed_dict)
                ...
                logger.info('validation step, mse = %.2f, current batch = %d, processed = %d', mse_value, actual_bsize, total_size)
            duration = default_timer() - start_time

            total_rmse = accumulated_mse / total_size
            logger.info('validation mse = %.2f%% (%.1f samples/sec; %.2f seconds)', otal_rmse, total_size / duration, duration)
...
def do_evaluation():
    ...
    # file:statements.meta.obj-->train/val/test.obj
    ds = data.load_dataset('../data/statements')

    # get node_type_num, besides word2int is the node_type_num
    # ds.word2int is the concrete type of node
    hyper.node_type_num = len(ds.word2int)  # node_type_number

    (compiler, _, _, _, raw_mse, batch_size_op) = build_model()
    ...
    with tf.Session() as sess:
        ...
        for shuffled in td.epochs(test_set, num_epochs):
            ...
            for step, batch in enumerate(td.group_by_batches(shuffled, hyper.batch_size), 1):
                ...
                mse_value, actual_bsize = sess.run([raw_mse, batch_size_op], feed_dict)
            ...
            total_mse = accumulated_mse / total_size

            logger.info('evaluation accusmulated mse = %.2f%% (%.1f samples/sec; %.2f seconds)', total_mse, total_size / duration, duration)
